...
export async function getCoverLetter(resumeId: string) {
  const supabase = await createClient();
...
export async function createCoverLetter(resumeId: string, richText: any = {}) {
  const supabase = await createClient();
  ...
  try {
    ...
    // Update the resume to indicate it has a cover letter
    await supabase
      .from('resumes')
...
export async function updateCoverLetter(resumeId: string, richText: any) {
  const supabase = await createClient();

  try {
    const { data, error } = await supabase
      .from('cover_letters')
      .update({ rich_text: richText })
      .eq('id', resumeId)
      .select()
      .single();
...
export async function deleteCoverLetter(resumeId: string) {
  const supabase = await createClient();
  ...
  try {
    ...
    // Update the resume to indicate it no longer has a cover letter
    await supabase
      .from('resumes')
