...
function TabButton({ value, children }: TabButtonProps) {
  return (
    <TabsTrigger 
      value={value}
      className="
        relative h-10 text-sm font-medium transition-all duration-300 ease-out
    ...
    >
      ...
      <div 
        className="
          absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-violet-500 to-blue-500 
          scale-x-0 data-[state=active]:scale-x-100 transition-transform duration-300 ease-out
        " 
      />
    </TabsTrigger>
  );
  ...
  return (
    <div className="space-y-4 mt-6">
      ...
      <Button
        variant="outline"
        className="
          w-full h-11 bg-white border-slate-200 hover:bg-slate-50 hover:border-slate-300
...
export function AuthDialog({ children }: AuthDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent 
        className="
          sm:max-w-[420px] p-0 bg-white border border-slate-200/80 shadow-2xl 
        ...
        <AuthProvider>
          <div className="px-6 pt-6 pb-4 text-center border-b border-slate-100">
            ...
            <div className="flex items-center justify-center space-x-2 mb-3">
              <div className="p-1.5 rounded-lg bg-gradient-to-br from-violet-500/10 to-blue-500/10">
            ...
            <Logo className="text-2xl text-slate-900" asLink={false} />
          ...
          >
            <TabsList className="
              w-full h-12 bg-slate-50/50 border-0 p-1 mx-6 mt-4 mb-0
              grid grid-cols-2 gap-0 rounded-xl
            ">
