...
export async function POST(req: Request) {
  try {
    ...
    const result = streamText({
      ...
      1. Read the resume using the getResume tool with either:
         - {"sections": "all"} for the full resume
         - {"sections": ["education"]} for a single section
         - {"sections": ["education", "work_experience"]} for multiple sections
      2. Acknowledge the resume was read, for example:
      "I have read your resume, and ..."
      3. If the user JUST simply asks you to read the resume, use {"sections": "all"} 
      then briefly note a few strengths and weaknesses.
      4. If the user asks you to read a specific section, use the tool with an array 
      of section names, e.g., {"sections": ["work_experience"]}
      5. If the user asks you to read a section, then use a tool, just mentioned that you read 
      it and now you will perform the action. 
      6. use the getResume tool freely, as much as needed. 
      7. Always format the sections parameter correctly:
         - Use "all" as a string for full resume
         - Use arrays for specific sections: ["education"], ["skills"], etc.
